...
export default function AdminPanel({
  ...
  return (
    <aside className="w-80 min-w-[280px] flex-shrink-0 bg-slate-900 border-r border-slate-700/50 flex flex-col h-screen overflow-y-auto shadow-2xl">
      ...
      {/* ── Action Buttons ── */}
      <div className="px-5 py-4 border-t border-slate-700/60 bg-slate-900/80 flex flex-col gap-3">
        <div className="flex gap-2">
          <button
            onClick={onPresent}
            disabled={appMode === 'bible' ? !hasAnyVerse : (lyricsSlides && lyricsSlides.length === 0)}
            className={`flex-1 py-3.5 rounded-xl font-bold text-xs transition-all flex items-center justify-center gap-2 shadow-lg ${
              ((appMode === 'bible' && hasAnyVerse) || (appMode === 'lyrics' && lyricsSlides && lyricsSlides.length > 0))
        ...
        {!hasAnyVerse && appMode === 'bible' && (
          <p className="text-center text-[10px] text-slate-600">
            Fetch a verse to enable presentation
          </p>
        )}
        {lyricsRawText.trim().length === 0 && appMode === 'lyrics' && (
          <p className="text-center text-[10px] text-slate-600">
            Paste lyrics to enable presentation
          </p>
        )}
      </div>
